...
class L_con(nn.Module):
    def __init__(self, patch_size=16, min_brightness=0.4, max_brightness=0.9):
        super().__init__()
        self.pool = nn.AvgPool2d(kernel_size=patch_size)
    ...
    def forward(self, x, t):
        # 평균 밝기 목표: 0.4 ~ 0.9 선형 매핑
        target = self.min_brightness + \
            (self.max_brightness - self.min_brightness) * \
            t.view(-1, 1, 1, 1)  # (B, 1, 1, 1)
        mean = self.pool(torch.mean(x, dim=1, keepdim=True))  # (B, 1, H', W')
