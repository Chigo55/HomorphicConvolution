...
class HomomorphicDitDCE(HomomorphicDit):
    ...
    def forward(self, x, t):
        ...
        dit_enh_img = self.ycrcb2rgb(YCrCb)
        dit_enh_img = torch.clamp(dit_enh_img, 0.0, 1.0)
        dce_enh_img, r = self.dce(x, dit_enh_img)
        return dce_enh_img, dit_enh_img, r
...
class HomomorphicDiTLightning(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_spa = self.lambda_spa * torch.mean(self.spa_loss(x, enh_img))
        loss_col = self.lambda_col * torch.mean(self.col_loss(enh_img))
        loss_exp = self.lambda_exp * torch.mean(self.exp_loss(enh_img))
...
class HomomorphicDiTDCELightning(L.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        dce_enh_img, dit_enh_img, r = self(x, t)
        ...
        total = (loss_spa + loss_col + loss_exp + loss_tva)
